...
## Part 2
Part 2 udvider netværket med central DHCP på R1 og DHCP relay på R2.

### part2_vsrxR1.json
- Beskrivelse: Konfiguration for vSRX **R1** (host-name: `R1`) som master DHCP server.
- Interfaces / net:
  - `ge-0/0/1.0` → `192.168.1.1/24` (Net1)
  - `ge-0/0/2.0` → `192.168.2.1/24` (Net2)
  - `ge-0/0/3.0` → `10.10.10.10/24` (transit/link mod R2)
- Services:
  - SSH aktiveret
  - DHCP lokal server via `dhcp-local-server` for Net1, Net2 og relay-trafik på `ge-0/0/3.0`
  - DHCP pools via `access address-assignment`:
    - Net1: `192.168.1.10–192.168.1.100`, router `192.168.1.1`, DNS `8.8.8.8`, max-lease `600`
    - Net2: `192.168.2.10–192.168.2.100`, router `192.168.2.1`, DNS `8.8.8.8`, max-lease `600`
    - Net3: `192.168.3.10–192.168.3.100`, router `192.168.3.1`, DNS `8.8.8.8`, max-lease `600`
    - Net4: `192.168.4.10–192.168.4.100`, router `192.168.4.1`, DNS `8.8.8.8`, max-lease `600`
- Routing:
  - Statiske routes til Net3 (`192.168.3.0/24`) og Net4 (`192.168.4.0/24`) via next-hop `10.10.10.11`
- Security:
  - `trust → trust` policy `default-permit`
  - Host-inbound tillader `ping`, `ssh` og `dhcp` på LAN- og transit-interfaces

### part2_vsrxR2.json
- Beskrivelse: Konfiguration for vSRX **R2** (host-name: `R2`) som DHCP relay mod R1.
- Interfaces / net:
  - `ge-0/0/1.0` → `192.168.3.1/24` (Net3)
  - `ge-0/0/2.0` → `192.168.4.1/24` (Net4)
  - `ge-0/0/3.0` → `10.10.10.11/24` (transit/link mod R1)
- Services:
  - SSH aktiveret
  - DHCP relay via `forwarding-options helpers bootp` til server `10.10.10.10`
  - Relay interfaces: `ge-0/0/1` og `ge-0/0/2`
- Routing:
  - Statiske routes til Net1 (`192.168.1.0/24`) og Net2 (`192.168.2.0/24`) via next-hop `10.10.10.10`
- Security:
  - `trust → trust` policy `default-permit`
