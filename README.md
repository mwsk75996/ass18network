...
- Beskrivelse: Konfiguration for vSRX **R2** (host-name: `R2`) – Challenge 8.
- Interfaces / net:
  - `ge-0/0/1.0` → `192.168.3.1/24` (Net3)
  - `ge-0/0/2.0` → `192.168.4.1/24` (Net4)
  - `ge-0/0/3.0` → `10.10.10.11/24` (transit/link mod R1)
- Services:
  ...
  - DHCP server via `system services dhcp` (pools for Net3 og Net4)
  - DHCP pools:
    - Net3: `192.168.3.10–192.168.3.100`, router `192.168.3.1`, DNS `8.8.8.8`, lease `86400`
    - Net4: `192.168.4.10–192.168.4.100`, router `192.168.4.1`, DNS `8.8.8.8`, lease `86400`
- Routing:
  - Statiske routes til Net1 (`192.168.1.0/24`) og Net2 (`192.168.2.0/24`) via next-hop `10.10.10.10`
- Security:
  - `trust → trust` policy `default-permit`
  - Host-inbound tillader `ping`, `ssh` og `dhcp` på `ge-0/0/1.0` og `ge-0/0/2.0`, og `ping`/`ssh` på transit-interface `ge-0/0/3.0`
